#include <cmath>
#include <renderers/renderer_opengl_immediate/renderer_opengl_immediate.hpp>
...
void renderer_opengl_immediate::present(){
    SDL_GL_SwapWindow(window);
}

void renderer_opengl_immediate::draw_texture(size_t arg, int x, int y){
    texture_gl_legacy* Texture = textures[arg];
    draw_texture_(Texture, x, y - static_cast<int>(Texture->height));
}

void renderer_opengl_immediate::draw_rectangle(int x, int y, int w, int h, SDL_Color color){
...
void renderer_opengl_immediate::draw_rectangle_filled(int x, int y, int w, int h, SDL_Color color){
	GLfloat col = 1.0f/255.0f;
	GLfloat r = col*(GLfloat)color.r, g = col*(GLfloat)color.g, b = col*(GLfloat)color.b, a = col*(GLfloat)color.a;
	auto X = static_cast<GLfloat>(x), Y = static_cast<GLfloat>(y), W = static_cast<GLfloat>(w), H = static_cast<GLfloat>(h);
	GLfloat max_x = X + W;
...
void renderer_opengl_immediate::draw_background(size_t arg){
    texture_gl_legacy* Texture = textures[arg];
    if(Texture != nullptr){
        glEnable(GL_TEXTURE_2D);
...
void renderer_opengl_immediate::draw_sprite(size_t arg, int x, int y, int sprite, int animation, int animation_num, int sprite_num){
    texture_gl_legacy* Texture = textures[arg];
    if(Texture != nullptr){
        glEnable(GL_TEXTURE_2D);
        glBindTexture(GL_TEXTURE_2D, Texture->TextureID);
...
void renderer_opengl_immediate::draw_overlay(size_t arg, int sprite, int sprite_num){
    texture_gl_legacy* Texture = textures[arg];
    if(Texture != nullptr){
        glEnable(GL_TEXTURE_2D);
        int animation_num = 1;
...
void renderer_opengl_immediate::clear_screen(){
    glClearColor(0,0,0,1);
    glClear( GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
}

//Text fallback for non-english text -- works with cyrillic, expanded Latin (Spanish, German, etc..) and I guess all of UTF8
void renderer_opengl_immediate::draw_text_normal(std::string arg, std::string arg2, int x, int y, SDL_Color color, int size){
...
texture_gl_legacy* renderer_opengl_immediate::create_texture_from_surface(SDL_Surface* arg){
    glEnable(GL_TEXTURE_2D);
...
void renderer_opengl_immediate::draw_text_cached(std::string arg, std::string arg2, int x, int y, SDL_Color color , int size){
    std::string font_and_size = arg+std::to_string(size);
...
void renderer_opengl_immediate::draw_texture_(texture_gl_legacy* Texture, int x, int y){
    if(Texture != nullptr){
        glEnable(GL_TEXTURE_2D);
        glBindTexture(GL_TEXTURE_2D, Texture->TextureID);
...
void renderer_opengl_immediate::draw_text(std::string arg, std::string arg2, int x, int y, SDL_Color color_font , int size, int flag){
    if(flag == 1){
...
void renderer_opengl_immediate::set_draw_color(Uint8 r, Uint8 g, Uint8 b, Uint8 a){
    glColor4f((1.0f/255)*r, (1.0f/255)*g, (1.0f/255)*b, (1.0f/255)*a);

}

GLuint renderer_opengl_immediate::new_texture_id(){
    return texture_ids++;
}

void renderer_opengl_immediate::close() {
    for(auto& it : fonts){
...
void renderer_opengl_immediate::upload_surfaces(std::unordered_map<size_t, SDL_Surface*>* surfaces){
    if(surfaces != nullptr){
...
void renderer_opengl_immediate::upload_fonts(std::unordered_map<std::string, TTF_Font*>* fonts){
    if(fonts != nullptr){
        for ( auto& it : *fonts){
            if(it.second == nullptr){
                for(auto& k : fonts_cache[it.first]){
...
void renderer_opengl_immediate::cache_font(TTF_Font* Font, std::string font_and_size){
...
void renderer_opengl_immediate::vsync_on(){
    SDL_GL_SetSwapInterval(1);
}

void renderer_opengl_immediate::vsync_off(){
