...
void drawing_manager::handle_messages(){
    ...
    while(temp != nullptr){
        if(temp->msg_name == SET_VSYNC){
            ...
            set_darkness(static_cast<uint8_t>(temp->base_data));
        }
        else if(temp->msg_name == SHOW_COLLISIONS){
            collisions_shown = static_cast<bool>(temp->base_data);
        }
        else if(temp->msg_name == SHOW_FPS){
            show_fps = static_cast<bool>(temp->base_data);
        }
        else if(temp->msg_name == ENABLE_LIGHTING){
            lighting_enabled = static_cast<bool>(temp->base_data);
        }
