...
int renderer_opengl::initialize(SDL_Window* window, int v_width, int v_height){
    //initialize SDL_TTF
    if(TTF_Init() < 0){
...
GLuint renderer_opengl::create_vbo(GLfloat* vertices, int size){
	GLuint ID;
	glGenBuffers(1, &ID);
...
void renderer_opengl::draw_texture(size_t arg, int x, int y){
...
void renderer_opengl::draw_rectangle(int x, int y, int w, int h, SDL_Color color){
...
void renderer_opengl::draw_background(size_t arg){
	/*texture* Texture = textures[arg];
...
void renderer_opengl::draw_sprite(size_t arg, int x, int y, int sprite, int animation, int animation_num, int sprite_num){
    /*texture* Texture = textures[arg];
    if(Texture != NULL){
        glEnable(GL_TEXTURE_2D);
        glBindTexture(GL_TEXTURE_2D, Texture->TextureID);
...
void renderer_opengl::draw_overlay(size_t arg, int sprite, int sprite_num){
    /*texture* Texture = textures[arg];
    if(Texture != NULL){
        glEnable(GL_TEXTURE_2D);
        int animation_num = 1;
...
void renderer_opengl::clear_screen(){
...
void renderer_opengl::draw_text_normal(std::string arg, std::string arg2, int x, int y, SDL_Color color, int size){
...
texture* renderer_opengl::add_to_atlas(SDL_Surface* arg){
    glEnable(GL_TEXTURE_2D);
...
void renderer_opengl::draw_text_cached(std::string arg, std::string arg2, int x, int y, SDL_Color color , int size){
    /*std::string font_and_size = arg+std::to_string(size);
...
void renderer_opengl::draw_texture_(texture* Texture, int x, int y){
	/*if(Texture != NULL){
		// 1rst attribute buffer : vertices
		glEnableVertexAttribArray(0);
...
void renderer_opengl::draw_text(std::string arg, std::string arg2, int x, int y, SDL_Color color_font , int size, int flag){
    /*if(flag == 1){
...
void renderer_opengl::set_draw_color(Uint8 r, Uint8 g, Uint8 b, Uint8 a){
    glColor4f((1.0f/255)*r, (1.0f/255)*g, (1.0f/255)*b, (1.0f/255)*a);
}

GLuint renderer_opengl::new_texture_id(){
    return id++;
}

void renderer_opengl::build_atlas(){
...
void renderer_opengl::upload_surfaces(std::unordered_map<size_t, SDL_Surface*>* surfaces){
	vbo_index = 0;
	if(surfaces != nullptr){
...
void renderer_opengl::upload_fonts(std::unordered_map<std::string, TTF_Font*>* fonts){
    /*if(fonts != NULL)
...
void renderer_opengl::cache_font(TTF_Font* Font, std::string font_and_size){
...
void renderer_opengl::vsync_on(){
    SDL_GL_SetSwapInterval(1);
}

void renderer_opengl::vsync_off(){
