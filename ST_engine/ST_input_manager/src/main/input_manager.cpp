 ...
 */
input_manager::input_manager(task_manager &gTask_manager, message_bus &gMessageBus) : gMessage_bus(gMessageBus),
    ...
    if (SDL_Init(SDL_INIT_JOYSTICK | SDL_INIT_GAMECONTROLLER | SDL_INIT_HAPTIC) < 0) {
        gMessage_bus.send_msg(
                new message(LOG_ERROR, make_data<std::string>("Could not initialize gamepad subsystem!")));
    }
    ...
    while (SDL_PollEvent(&event) != 0) {
        ...
        if (event.cdevice.type == SDL_CONTROLLERDEVICEADDED) {
            ...
            gMessage_bus.send_msg(new message(LOG_INFO, make_data<std::string>(
                    "Found a controller: " + std::string(SDL_GameControllerName(controller)))));
            SDL_Haptic *haptic = SDL_HapticOpen(static_cast<int32_t>(controllers.size() - 1));
            if (haptic != nullptr) {
                if (SDL_HapticRumbleInit(haptic) < 0) {
                    gMessage_bus.send_msg(new message(LOG_INFO, make_data<std::string>(
                            "Unable to initialize rumble for controller " +
                            std::string(SDL_GameControllerName(controller)))));
                } else {
                    gMessage_bus.send_msg(new message(LOG_INFO, make_data<std::string>(
        ...
        if (event.cdevice.type == SDL_CONTROLLERDEVICEREMOVED) {
            ...
            gMessage_bus.send_msg(new message(LOG_INFO, make_data<std::string>(
                    "Controller " + std::to_string(number + 1) + " disconnected")));
        }
    }
...
void input_manager::take_controller_input() {
    ...
    for (SDL_GameController *c: controllers) {
        controls.buttons[static_cast<uint8_t>(ST::key::CONTROLLER_BUTTON_A)] = SDL_GameControllerGetButton(c,
                                                                                                           SDL_CONTROLLER_BUTTON_A);
        controls.buttons[static_cast<uint8_t>(ST::key::CONTROLLER_BUTTON_B)] = SDL_GameControllerGetButton(c,
                                                                                                           SDL_CONTROLLER_BUTTON_B);
        controls.buttons[static_cast<uint8_t>(ST::key::CONTROLLER_BUTTON_X)] = SDL_GameControllerGetButton(c,
                                                                                                           SDL_CONTROLLER_BUTTON_X);
        controls.buttons[static_cast<uint8_t>(ST::key::CONTROLLER_BUTTON_Y)] = SDL_GameControllerGetButton(c,
                                                                                                           SDL_CONTROLLER_BUTTON_Y);
        controls.buttons[static_cast<uint8_t>(ST::key::CONTROLLER_BUTTON_DPAD_UP)] = SDL_GameControllerGetButton(c,
                                                                                                                 SDL_CONTROLLER_BUTTON_DPAD_UP);
        controls.buttons[static_cast<uint8_t>(ST::key::CONTROLLER_BUTTON_DPAD_DOWN)] = SDL_GameControllerGetButton(c,
                                                                                                                   SDL_CONTROLLER_BUTTON_DPAD_DOWN);
        controls.buttons[static_cast<uint8_t>(ST::key::CONTROLLER_BUTTON_DPAD_LEFT)] = SDL_GameControllerGetButton(c,
                                                                                                                   SDL_CONTROLLER_BUTTON_DPAD_LEFT);
        controls.buttons[static_cast<uint8_t>(ST::key::CONTROLLER_BUTTON_DPAD_RIGHT)] = SDL_GameControllerGetButton(c,
                                                                                                                    SDL_CONTROLLER_BUTTON_DPAD_RIGHT);
        controls.buttons[static_cast<uint8_t>(ST::key::CONTROLLER_BUTTON_LEFTSTICK)] = SDL_GameControllerGetButton(c,
                                                                                                                   SDL_CONTROLLER_BUTTON_LEFTSTICK);
        controls.buttons[static_cast<uint8_t>(ST::key::CONTROLLER_BUTTON_RIGHTSTICK)] = SDL_GameControllerGetButton(c,
                                                                                                                    SDL_CONTROLLER_BUTTON_RIGHTSTICK);
        controls.buttons[static_cast<uint8_t>(ST::key::CONTROLLER_BUTTON_LEFTSHOULDER)] = SDL_GameControllerGetButton(c,
                                                                                                                      SDL_CONTROLLER_BUTTON_LEFTSHOULDER);
        controls.buttons[static_cast<uint8_t>(ST::key::CONTROLLER_BUTTON_RIGHTSHOULDER)] = SDL_GameControllerGetButton(
                c, SDL_CONTROLLER_BUTTON_RIGHTSHOULDER);
        controls.buttons[static_cast<uint8_t>(ST::key::CONTROLLER_BUTTON_START)] = SDL_GameControllerGetButton(c,
                                                                                                               SDL_CONTROLLER_BUTTON_START);
        controls.buttons[static_cast<uint8_t>(ST::key::CONTROLLER_BUTTON_SELECT)] = SDL_GameControllerGetButton(c,
        ...
        controls.buttons[static_cast<uint8_t>(ST::key::CONTROLLER_BUTTON_LEFT_TRIGGER)] =
                controller_analog_inputs.left_trigger > 0;
        controls.buttons[static_cast<uint8_t>(ST::key::CONTROLLER_BUTTON_RIGHT_TRIGGER)] =
                controller_analog_inputs.right_trigger > 0;
    }
    //only send controller axis values if they change and the values exceed the thresholds

    //Branch-less checks
    controller_analog_inputs.left_trigger = (controller_analog_inputs.left_trigger >= left_trigger_threshold) *
                                            controller_analog_inputs.left_trigger; // NOLINT(cppcoreguidelines-narrowing-conversions)
    controller_analog_inputs.right_trigger = (controller_analog_inputs.right_trigger >= right_trigger_threshold) *
                                             controller_analog_inputs.right_trigger; // NOLINT(cppcoreguidelines-narrowing-conversions)
    ...
    //Branch-less check
    controller_analog_inputs.left_stick_vertical = (controller_analog_inputs.left_stick_vertical >
                                                    left_stick_vertical_threshold // NOLINT(cppcoreguidelines-narrowing-conversions)
                                                    || controller_analog_inputs.left_stick_vertical <
                                                       -left_stick_vertical_threshold) *
                                                   controller_analog_inputs.left_stick_vertical;

    //Branch-less check
    controller_analog_inputs.left_stick_horizontal = (controller_analog_inputs.left_stick_horizontal >
                                                      left_stick_horizontal_threshold // NOLINT(cppcoreguidelines-narrowing-conversions)
                                                      || controller_analog_inputs.left_stick_horizontal <
                                                         -left_stick_horizontal_threshold) *
                                                     controller_analog_inputs.left_stick_horizontal;

    //Branch-less check
    controller_analog_inputs.right_stick_horizontal = (controller_analog_inputs.right_stick_horizontal >
                                                       right_stick_horizontal_threshold // NOLINT(cppcoreguidelines-narrowing-conversions)
                                                       || controller_analog_inputs.right_stick_horizontal <
                                                          -right_stick_horizontal_threshold) *
                                                      controller_analog_inputs.right_stick_horizontal;

    //Branch-less check
    controller_analog_inputs.right_stick_vertical = (controller_analog_inputs.right_stick_vertical >
                                                     right_stick_vertical_threshold // NOLINT(cppcoreguidelines-narrowing-conversions)
                                                     || controller_analog_inputs.right_stick_vertical <
                                                        -right_stick_vertical_threshold) *
                                                    controller_analog_inputs.right_stick_vertical;
    ...
    while (temp != nullptr) {
        switch (temp->msg_name) {
            ...
            default: //Otherwise we process any threshold messages
                ...
                this->left_stick_horizontal_threshold = left_stick_hor_check * static_cast<int16_t>(temp->base_data0) +
                ...
                this->left_stick_vertical_threshold = left_stick_ver_check * static_cast<int16_t>(temp->base_data0) +
                                                      // NOLINT(cppcoreguidelines-narrowing-conversions)
                ...
                this->right_stick_horizontal_threshold =
                        right_stick_hor_check * static_cast<int16_t>(temp->base_data0) +
                        // NOLINT(cppcoreguidelines-narrowing-conversions)
                ...
                this->right_stick_vertical_threshold = right_stick_ver_check * static_cast<int16_t>(temp->base_data0) +
                                                       // NOLINT(cppcoreguidelines-narrowing-conversions)
                ...
                this->right_trigger_threshold = right_trigger_check * static_cast<int16_t>(temp->base_data0) +
                                                // NOLINT(cppcoreguidelines-narrowing-conversions)
                                                !right_trigger_check * this->right_trigger_threshold;

                uint8_t left_trigger_check = (temp->msg_name == SET_LEFT_TRIGGER_THRESHOLD);
                this->left_trigger_threshold = left_trigger_check * static_cast<int16_t>(temp->base_data0) +
                                               // NOLINT(cppcoreguidelines-narrowing-conversions)
