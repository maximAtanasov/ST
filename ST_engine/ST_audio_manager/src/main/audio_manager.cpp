 ...
 */
audio_manager::audio_manager(task_manager &gTask_manager, message_bus &gMessageBus) : gMessage_bus(gMessageBus),
...
void audio_manager::play_sound(uint16_t arg, uint8_t volume, int8_t loops) const {
    ...
    if (!muted) { //null-check not necessary in the newest version of SDL_Mixer
    ...
    if (Mix_PlayChannel(-1, chunk, loops) == -1) {
        gMessage_bus.send_msg(
                new message(LOG_ERROR, make_data<std::string>("Mix_PlayChannel Error " + std::string(Mix_GetError()))));
    }
}
...
void audio_manager::play_music(uint16_t arg, uint8_t volume, int8_t loops) const {
    ...
    if (Mix_PlayMusic(music, loops) == -1) {
        gMessage_bus.send_msg(
                new message(LOG_ERROR, make_data<std::string>("Mix_PlayMusic Error " + std::string(Mix_GetError()))));
    }
}
