...
TEST_F(asset_manager_test, loadPNG) {
    msg_bus.send_msg(make_msg(LOAD_ASSET, make_data<std::string>("../src/test/assets_manager/test_image.png")));
    update_task(&test_mngr);
...
TEST_F(asset_manager_test, loadWAV) {
    msg_bus.send_msg(make_msg(LOAD_ASSET, make_data<std::string>("../src/test/assets_manager/test_sound.wav")));
    update_task(&test_mngr);
    Mix_Chunk* expected_chunk = Mix_LoadWAV("../src/test/assets_manager/test_sound.wav");
...
TEST_F(asset_manager_test, loadOGG) {
    msg_bus.send_msg(make_msg(LOAD_ASSET, make_data<std::string>("../src/test/assets_manager/test_music.ogg")));
    update_task(&test_mngr);
...
TEST_F(asset_manager_test, loadTTF_nonExistant) {
    msg_bus.send_msg(make_msg(LOAD_ASSET, make_data<std::string>("../src/test/assets_manager/nothing.ttf 50")));
    update_task(&test_mngr);
    EXPECT_EQ(nullptr, get_assets().fonts["test_font.ttf"]);
}

TEST_F(asset_manager_test, loadTTF_noSize) {
    msg_bus.send_msg(make_msg(LOAD_ASSET, make_data<std::string>("../src/test/assets_manager/test_font.ttf")));
    update_task(&test_mngr);
    EXPECT_EQ(nullptr, get_assets().fonts["test_font.ttf"]);
}

TEST_F(asset_manager_test, loadTTF) {
    msg_bus.send_msg(make_msg(LOAD_ASSET, make_data<std::string>("../src/test/assets_manager/test_font.ttf 50")));
    update_task(&test_mngr);
    TTF_Font* expected_font = TTF_OpenFont("../src/test/assets_manager/test_font.ttf", 50);
